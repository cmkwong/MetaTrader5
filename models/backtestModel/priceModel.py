...
def get_Prices(symbols, timeframe, timezone, start=None, end=None, count=10, deposit_currency='USD'):
    ...
    q2d_name = "q2d"
    q2d_exchange_rate_df, q2d_modified_names = exchgModel.get_exchange_df(symbols, all_symbols_info, deposit_currency, timeframe, timezone, '1000', count, exchg_type=q2d_name, col_names=[q2d_name]*len(symbols), start=start, end=end)

    # get the base to deposit exchange rate
    b2d_name = "b2d"
    b2d_exchange_rate_df, b2d_modified_names = exchgModel.get_exchange_df(symbols, all_symbols_info, deposit_currency, timeframe, timezone, '1000', count, exchg_type=b2d_name, col_names=[b2d_name]*len(symbols), start=start, end=end)
...
def get_latest_Prices(all_symbols_info, symbols, timeframe, timezone, count=10, deposit_currency='USD'):
    ...
    q2d_name = "q2d"
    q2d_exchange_rate_df_o, q2d_modified_names = exchgModel.get_exchange_df(symbols, all_symbols_info, deposit_currency, timeframe, timezone, '1000', count, exchg_type=q2d_name, col_names=[q2d_name]*len(symbols))
    q2d_exchange_rate_df_c, _ = exchgModel.get_exchange_df(symbols, all_symbols_info, deposit_currency, timeframe, timezone, '0001', count, exchg_type=q2d_name, col_names=[q2d_name]*len(symbols))
