...
with mt5Model.Helper():
    ...
    # read extra data
    long_signal, short_signal, long_q2d, short_q2d = fileModel.read_min_extra_info(file_options['extra_data_path'])
    ...
    for symbol_count, symbol in enumerate(file_options['symbols']):
        ...
        symbol_df = symbol_df[~symbol_df.index.duplicated(keep='first')] # note 80b and note 81c

        signalModel.get_resoluted_signal(long_signal, symbol_df.index)
    ...
    # write the csv
    csv_name = "{}_test.csv".format(DT_STRING)
    print("Writing csv to {}".format(file_options['output_path']))
    symbols_df.to_csv(os.path.join(file_options['output_path'], csv_name))
    print()